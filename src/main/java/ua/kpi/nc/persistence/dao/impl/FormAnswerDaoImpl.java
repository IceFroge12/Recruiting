...
public class FormAnswerDaoImpl extends JdbcDaoSupport implements FormAnswerDao {
    private static final String SQL_GET_BY_ID =
            "SELECT fa.id, fa.answer, fa.id_question, fa.id_application_form, fa.id_variant, fa.id_interview \n" +
                    "FROM form_answer fa \n" +
                    "WHERE fa.id = ?;";
    private static final String SQL_GET_BY_INTERVIEW_AND_QUESTION =
            "SELECT fa.id, fa.answer, fa.id_question, fa.id_application_form, fa.id_variant, fa.id_interview \n" +
                    "FROM form_answer fa \n" +
                    "WHERE fa.id_interview= ? and  fa.id_question = ?;";
    private static final String SQL_INSERT =
            "INSERT INTO form_answer(answer, id_question, id_application_form, id_variant, id_interview) \n" +
                    "VALUES (?,?,?,?,?);";
    ...
    public FormAnswer getById(Long id) {
        ...
        return this.getJdbcTemplate().queryWithParameters(SQL_GET_BY_ID, new FormAnswerDaoImpl.FormAnswerExtractor(), id);
    }

    @Override
    public Set<FormAnswer> getByInterviewAndQuestion(Interview interview, FormQuestion question) {
        if (log.isInfoEnabled()) {
            log.info("Looking for answer with interview_id= " + interview.getId() + "and question= " + question.getTitle());
        }
    ...
    public Long insertFormAnswer(FormAnswer formAnswer, Interview interview, FormQuestion question,
        ...
        if (log.isInfoEnabled()) {
            log.info("Inserting form answer with interview_id= " + interview.getId() + ", question_id= " + question.getTitle() +
                    ", application form id=" + applicationForm.getId() + ", form answer variant id= " + answerVariant.getId());
        }
